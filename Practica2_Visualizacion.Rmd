...
grafico2
grafico3 = grafico2 + ggtitle("Dispersion de temperatura máxima y mínima")
grafico3
ggsave("./figure/Ejercicio1.pdf",height = 5, width = 5)
```
...
grafico1 = ggplot(data = ds,aes(x = Temp9am,y=Pressure9am)) +
  geom_point()
grafico1
grafico1 + geom_point(aes(shape = RainToday))
grafico1 + geom_point(aes(col = RainToday))
#Como podemos ver se ve mucho mejor con la diferencia de color
grafico2 = grafico1 + geom_point(aes(col = RainToday))
grafico2 + ggtitle("Relación entre la temperatura y la presión")
ggsave("./figure/Ejercicio2.pdf",height = 5, width = 5)
```
...
str(iris.tidy)
#Como vemos hay Species, Part, Measure, Value
iris.grafico1 = ggplot(data = iris.tidy, aes(x = Species, y = Value))
iris.grafico1 + geom_point()
iris.grafico2 = iris.grafico1 + geom_point(aes(col = Part))
iris.grafico2 + facet_grid(Measure~.)
iris.grafico2 + facet_grid(.~Measure)
#Yo creo que se ve mejor en columnas, aunque no exsiste mucha diferencia
iris.grafico1 + 
  geom_jitter(aes(col = Part)) +
  facet_grid(.~Measure)
ggsave("./figure/Ejercicio3.pdf",height = 5, width = 5)
```
...
str(DatosEjer4)
#Como vemos hay Fecha, Fabricante y Ventas
grafico_base = ggplot(data = DatosEjer4, aes(x = Fecha, y = Ventas, col = Fabricante))
grafico_base + geom_point()
#No se ve mal del todo pero con lineas se vería mejor
grafico_base + geom_line()
lineas = grafico_base + geom_line(aes(linetype = Fabricante))
lineas
lineas + ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
ggsave("./figure/Ejercicio4.pdf",height = 5, width = 5)
```
...
str(Experimento)
#Como vemos hay Complemento, Dosis, Altura y Error_Altura
base = ggplot(data = Experimento, aes(x = Dosis, y = Altura,shape = Complemento,linetype = Complemento)) + 
  geom_point() + 
  geom_line()
base
base + geom_errorbar(aes(ymin = Altura-Error_Altura, ymax = Altura+Error_Altura))
ggplot(data = Experimento, aes(x = Dosis, y = Altura, col = Complemento)) + 
  geom_point(aes(shape = Complemento)) + 
  geom_line(aes(linetype = Complemento)) +
  geom_errorbar(aes(ymin = Altura-Error_Altura, ymax = Altura+Error_Altura,linetype = Complemento)) + 
  scale_color_brewer(palette="Reds")
ggsave("./figure/Ejercicio5.pdf",height = 5, width = 5)
```
...
ggplot(data = ds)+
  geom_boxplot(aes(x=Year,y = MinTemp))
violin = ggplot(data = ds, aes(fill = Year))+
  geom_violin(aes(x=Year,y = MaxTemp))+
  labs(title = "Distribución de temperaturas máximas por año",y="Temperatura (ºC)",x = "Año")
#Lo coloreamos para en el ejercicio siguiente quitar la leyenda
violin
violin + theme(axis.text.x = element_text(angle = 45),legend.position = "none")
ggsave("./figure/Ejercicio6.pdf",height = 5, width = 5)
```
...
download.file(url = "http://assets.datacamp.com/blog_assets/chol.txt", destfile = "./data/datacamp.txt")
df <-read.table("./data/datacamp.txt", header = T)
base = ggplot(data = df, aes(x = HEIGHT)) + 
              geom_histogram()
base
titulo = base + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia")
titulo
...
ggplot(data = df, aes(x = HEIGHT)) + 
  labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia") + 
  geom_histogram(breaks = seq(155,195,by =2),col = "red", fill = "blue",alpha=0.5,aes(y = ..density..)) + 
...
ggplot(data = df, aes(x = HEIGHT)) + 
  ...
  geom_density(col = "green") + 
  facet_grid(.~SMOKE)
ggsave("./figure/Ejercicio7.pdf",height = 5, width = 5)
```
